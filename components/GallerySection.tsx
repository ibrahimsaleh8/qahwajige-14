"use client";
import { GalleryImageData } from "@/lib/responseType";
import Image from "next/image";
import { motion } from "motion/react";

export function GallerySection({ gallery }: { gallery: GalleryImageData[] }) {
  return (
    <section
      id="gallery"
      className="py-24 relative overflow-hidden bg-second-bg">
      <div className="container mx-auto px-4 relative">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto">
          <p className="text-black mx-auto w-fit text-lg font-bold mb-4 p-3 bg-main-color">
            معرض الأعمال
          </p>
          <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">
            من ذكريات مناسباتنا{" "}
          </h2>
          <p className="text-white/80 text-lg max-w-2xl mx-auto">
            لقطات حية من فعاليات ومناسبات قمنا بخدمتها في الرياض
          </p>
          <div className="w-20 h-1 bg-main-color mx-auto my-6" />
        </div>

        {/* Gallery Grid */}
        <div className="grid grid-cols-2 md:grid-cols-3">
          {gallery.map((image, index) => (
            <motion.div
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.5, delay: index * 0.1 }}
              viewport={{ once: true }}
              key={index}
              className={`relative bg-second-bg overflow-hidden cursor-pointer group shadow-soft border-2 transition-all duration-300`}>
              <div className={`aspect-square`}>
                <Image
                  src={image.url}
                  alt={image.alt ?? `صورة-${index + 1}`}
                  width={1000}
                  height={1000}
                  className="w-full h-full object-cover transition-transform duration-500 "
                />
              </div>
              {/* Overlay */}
              <div className="absolute inset-0  group-hover:bg-[hsl(var(--coffee-dark)/0.4)] transition-colors duration-300 flex items-center justify-center">
                <span className="text-[hsl(var(--cream))] opacity-0 group-hover:opacity-100 transition-opacity duration-300 font-semibold text-center px-2">
                  {image.alt ?? `صورة-${index + 1}`}
                </span>
              </div>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
}
